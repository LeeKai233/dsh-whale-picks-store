...
The 鲸选 store entry inside DSH: a settings section right below Agent Presets that shows the whale-picks shelves — suits and curated plugins with nine-axis ASCII meter charts, founder scores and notes, pass findings and one-click-copy install commands. Pure text UI, no emoji.
...
- Nine-axis ASCII meter chart per listed plugin (the nine machine goals, btop-style titled box and gradient meters), founder score and review notes, machine-pass findings and spec-gate status
